...
function analyze(min=0) {
    ...
    for (let evt of EVENTS) {
      ...
      if(Math.abs(cor)> min){
        arr.push(evt+": "+cor.toFixed(4)+" "+count(evt));
      }
...
function analyze2(min=0) {
  // return [...EVENTS]
   
    for (let event of EVENTS) {
    
      console.log(event + ":", phi(tableFor(event,JOURNAL)+" "+count2(event)));
...
function mymap() {
  for (let event of journalEvents(JOURNAL)) {
    let correlation = phi(tableFor(event, JOURNAL));
    if (correlation > 0.3 ) {
      console.log(correlation*3);
    }
  }
}

function getOccurrence(JOURNAL, value) {
  var count = 0;
  JOURNAL.forEach((v) => (v === value && count++));
  return count;
}
